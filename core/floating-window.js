	...
	function stopMove(e)
	{
		...
		kernel.browser.setCookie(activeWindow.id + '_attr',activeWindow.style.left.replace('px','') + ',' + activeWindow.style.top.replace('px','') + ',' + activeWindow.style.width.replace('px','') + ',' + activeWindowContent.style.height.replace('px','') + ',' + activeWindow.style.zIndex + ',' + state,50);
	}
	...
	function moveWindow(e)
	{
		...
		if(initResizeCounter>=10){
			...
			activeWindowContent.style.height = newHeight  + 'px';	
		     //Fix for themes
			document.getElementById('topCenterImageID'+activeWindow.id.replace(/dhtml_goodies_id/, "")).style.width = newWidth-8+'px';	
	...
	{
		switchElement(false,document.getElementById('dhtml_goodies_id' + this.id.replace(/[^\d]/g,'')));	
		document.body.removeChild(document.getElementById(activeWindow.id));
	}
	...
	function initWindows(e,divObj) {
		...
		for(var no=0;no<divs.length;no++){
			...
			if(divs[no].className=='dhtmlgoodies_window'){	
				...
				if(readSizeFromCookie)var cookiePos = kernel.browser.getCookie(divs[no].id + '_attr') + '';	else cookiePos = '';
				if(divObj)cookiePos='';
				...
				for(var no2=0;no2<subImages.length;no2++){
					...
					if(subImages[no2].className=='maximizeButton'){
						subImages[no2].id = divCounter;
						subImages[no2].onclick = function() { kernel.windowmanager.maximizeWindow('', this); }
					}	
